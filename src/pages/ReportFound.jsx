...
export default function ReportFound() {
  ...
  const handleSubmit = async (formData) => {
    ...
    const pet = await base44.entities.Pet.create({ ...formData, status: 'found' });
    await fireMatchingAlerts(pet);
      ...
      <div className="px-4 py-6" style={{ background: HERO_COLORS.background }}>
        <div className="max-w-6xl mx-auto rounded-2xl overflow-hidden flex flex-col md:flex-row" style={{ minHeight: '340px' }}>
          ...
          <div className="md:w-1/2 w-full min-h-48 md:min-h-0">
            <img src="https://media.base44.com/images/public/69a0f10efc1058c9e80d1210/e0c2c1aca_pexels-zakaria-boumliha-11865104.jpg"
            alt="Report found pet"
          ...
          <div className="md:w-1/2 w-full flex flex-col justify-center px-10 py-10" style={{ background: HERO_COLORS.panelBg }}>
            ...
            <h1 className="text-5xl font-black mb-4 leading-tight" style={{ color: HERO_COLORS.panelText }}>report a found pet</h1>
            <p className="text-lg max-w-sm leading-relaxed mb-6" style={{ color: HERO_COLORS.panelSubtext }}>found a stray? provide as many details as possible. your photos & descriptions can make the difference between a lost pet and a reunited family.</p>
            <Link to={createPageUrl('LostAndFound')}>
              <Button className="self-start h-11 px-6 font-semibold rounded-xl bg-[#af501d] hover:bg-[#8f3f15] text-white">
